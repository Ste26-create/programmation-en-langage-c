...
void inscription(struct infopersonnel etudiant[]) {
    ...
    do {
        ...
        for (int i = 0; i < 6; i++) {
            printf("\n%s\t", lesinfo[i]);
            switch (i) {
                case 0: scanf("%s", etudiant[j].nom); break;
                case 1: scanf(" %[^\n]", etudiant[j].prenom); break;
                case 2: scanf("%s", etudiant[j].date_de_naissance); break;
                case 3: scanf(" %[^\n]", etudiant[j].adresse); break;
                case 4: scanf(" %[^\n]", etudiant[j].niveau); break;
                case 5: scanf(" %[^\n]", etudiant[j].filiere); break;
            }
        }
        ...
        if (c == 'L' || c == 'l') {
            if (num == '1') { etudiantL1[nb_eleveL1] = etudiant[j]; classe = 0; nb_eleveL1++; }
            else if (num == '2') { etudiantL2[nb_eleveL2] = etudiant[j]; classe = 1; nb_eleveL2++; }
            else if (num == '3') { etudiantL3[nb_eleveL3] = etudiant[j]; classe = 2; nb_eleveL3++; }
        } else if (c == 'M' || c == 'm') {
            if (num == '1') { etudiantM1[nb_eleveM1] = etudiant[j]; classe = 3; nb_eleveM1++; }
            else if (num == '2') { etudiantM2[nb_eleveM2] = etudiant[j]; classe = 4; nb_eleveM2++; }
        } else if (c == 'D' || c == 'd') {
            if (num == '1') { etudiantD1[nb_eleveD1] = etudiant[j]; classe = 5; nb_eleveD1++; }
            else if (num == '2') { etudiantD2[nb_eleveD2] = etudiant[j]; classe = 6; nb_eleveD2++; }
        }
